...
def world_to_image(world, point: np.array) -> np.array:
    """transform point from world coordinates into image coordinates.

    TODO: Should this be included?
    """

    cam = world.get_vehicle().get_camera()
    cam_from_world = np.linalg.inv(transform_to_numpy(cam.get_transform()))  # 4x4 matrix
    image_from_cam = cam.calibration  # 3x3 matrix

    coords_cam = np.dot(cam_from_world, point)
    # right handed coordinate system (x right, y down, z forward)
    coords_cam = np.array([coords_cam[1], -coords_cam[2], coords_cam[0]]).T

    point_image = np.dot(image_from_cam, coords_cam)
    point_image[0] = point_image[0] / point_image[2]
    point_image[1] = point_image[1] / point_image[2]
